...
local function GetGitHubTimer()
    ...
    return GitHubTimerValue -- Retorna último valor válido
end
...
local function UpdateTimer()
    task.spawn(function()
        while _G.ScriptActive do
            -- Verifica GitHub a cada 5 minutos (300 segundos)
            if os.time() - LastGitHubCheck >= 300 then
                GitHubTimerValue = GetGitHubTimer()
                LastGitHubCheck = os.time()
                
                -- Se o timer do GitHub mudou, reinicia contagem
...
Tabs.Home:AddButton({
    ...
    Callback = function()
        setclipboard("https://github.com/SEU_USER/SEU_REPO")
    ...
    Callback = function()
        ...
        if newTimer and newTimer > 0 then
            ...
            Fluent:Notify({
                ...
                Content = "Timer sincronizado: " .. newTimer .. " segundos", 
                Duration = 3
            })
        else
            Fluent:Notify({
                Title = "Erro", 
                Content = "Não foi possível sincronizar com GitHub", 
                Duration = 3
            })
        end
    end
...
-- [[ 🎯 COMBAT TAB COM VERIFICAÇÃO DE TIMER ]]
...
local function CheckTimerAndExecute(callback)
    return function(value)
        if _G.TimerExpired then
            Fluent:Notify({
                Title = "⏰ TEMPO ESGOTADO",
                Content = "Funções bloqueadas. Aguarde atualização do timer no GitHub.",
                Duration = 5
            })
            return
        end
...
Tabs.Combat:AddToggle("FOVToggle", {
    ...
    Default = true,
    Callback = CheckTimerAndExecute(function(value)
        _G.Settings.Aimbot.FOVVisible = value
        FOVCircle.Visible = value and not _G.TimerExpired
    end)
})
...
Tabs.Combat:AddDropdown("TargetPart", {
    ...
    Values = {"Head", "Neck", "HumanoidRootPart", "UpperTorso", "LowerTorso"},
    Default = "Head",
    Callback = CheckTimerAndExecute(function(value)
...
Tabs.Combat:AddToggle("WallCheck", {
    ...
    Default = true,
    Callback = CheckTimerAndExecute(function(value)
        _G.Settings.Aimbot.WallCheck = value
    end)
})

Tabs.Combat:AddDropdown("AimbotKey", {
    Title = "Tecla de Ativação",
    Description = "Segure para ativar o aimbot",
    Values = {"Botão Direito", "Botão Esquerdo", "Shift", "Control", "Alt"},
    Default = "Botão Direito",
    Callback = CheckTimerAndExecute(function(value)
        local keyMap = {
            ["Botão Direito"] = Enum.UserInputType.MouseButton2,
            ["Botão Esquerdo"] = Enum.UserInputType.MouseButton1,
...
Tabs.Visuals:AddToggle("ESPSkeleton", {
    ...
    Default = false,
    Callback = CheckTimerAndExecute(function(value)
        _G.Settings.ESP.Skeleton = value
    end)
})

Tabs.Visuals:AddToggle("ESPHealth", {
    Title = "Barra de Vida (Vertical)",
    Default = false,
    Callback = CheckTimerAndExecute(function(value)
        _G.Settings.ESP.HealthBar = value
    end)
})

Tabs.Visuals:AddToggle("ESPHealthH", {
    Title = "Barra de Vida (Horizontal)",
    Default = false,
    Callback = CheckTimerAndExecute(function(value)
        _G.Settings.ESP.HealthBarHorizontal = value
    end)
})

Tabs.Visuals:AddToggle("ESPChams", {
    Title = "Chams (Destaque)",
    Default = false,
    Callback = CheckTimerAndExecute(function(value)
        _G.Settings.ESP.Chams = value
    end)
})
...
Tabs.Colors:AddColorpicker("TracerColor", {
    ...
    Callback = CheckTimerAndExecute(function(value)
        _G.Settings.ESP.TracerColor = value
    end)
})
...
Tabs.Colors:AddColorpicker("SkeletonColor", {
    ...
    Callback = CheckTimerAndExecute(function(value)
        _G.Settings.ESP.SkeletonColor = value
    end)
})
...
Tabs.Colors:AddColorpicker("ChamsColor", {
    ...
    Default = Color3.fromRGB(255, 255, 255),
    Callback = CheckTimerAndExecute(function(value)
        _G.Settings.ESP.ChamsColor = value
    end)
})

Tabs.Colors:AddColorpicker("HealthColor", {
    Title = "Cor da Barra de Vida",
    Default = Color3.fromRGB(0, 255, 0),
    Callback = CheckTimerAndExecute(function(value)
        _G.Settings.ESP.HealthColor = value
    end)
})
...
Tabs.Misc:AddToggle("AntiAFK", {
    Title = "Anti AFK",
    Description = "Evita ser desconectado",
    Default = true,
    Callback = CheckTimerAndExecute(function(value)
        _G.Settings.Misc.AntiAFK = value
    end)
})

Tabs.Misc:AddButton({
    Title = "❌ Destruir Interface",
    Description = "Remove completamente o hub",
    Callback = function()
        _G.ScriptActive = false
        FOVCircle:Remove()
        Window:Destroy()
