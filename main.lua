...
task.spawn(function()
    while _G.ScriptAtivo do
        ...
        local Restante = TempoLimite - TempoPassado
        ...
        local m, s = math.floor(Restante/60), math.floor(Restante%60)
        TimerPara:SetDesc(string.format("Sua chave online expira em: %02dm %02ds", m, s))
        task.wait(1)
    end
end)
...
Tabs.Settings:AddButton({Title = "Copy Script Link/Key", Callback = function() setclipboard("https://esternal-hubs.com") Fluent:Notify({Title = "Copied", Content = "Link saved to clipboard!", Duration = 2}) end})
Tabs.Settings:AddButton({Title = "Unload Entire Script", Callback = function() _G.ScriptAtivo = false FovCircle:Remove() Window:Destroy() ScreenGui:Destroy() end})

-- [[ 👁️ MOTOR DE ESP ORIGINAL ]]
...
Window:SelectTab(1)
Fluent:Notify({Title = "Esternal | Hubs", Content = "Script Online Carregado!", Duration = 5})
